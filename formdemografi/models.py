...
"""
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
file, bukan database, hrs dijadikan JSON sbelum pake ini
# https://stackoverflow.com/questions/670442/asynchronous-file-upload-to-amazon-s3-with-django/9799520
...
class Info1(models.Model):
    # id_pengguna = ForeignKey(User)
    id_pengguna = models.ForeignKey(
        'auth.User',
        # 'User', 
        related_name="pengguna_%(class)s_objects",
        null=True)
    
    status_edit = models.BooleanField(
        default=False,
        blank = False)
    status_sinkronisasi = models.BooleanField(
        default=False,
        blank = False)
    # changedSinceSync
    # date_uploaded
...
class tabel_sinkronisasi_client(Info1):
    ...
    class Meta:
        unique_together = (("id_pengguna", "id_transaksi_client"),)
...
class rekmed_pasien(models.Model):
    pasien = models.ForeignKey('Patient', related_name='patientcondition', null=True, on_delete=models.CASCADE)
...
# )
    ...
    # def humannametext(self):
    #     return self.patientname.objects.latest('date_created').date_created


# =========================================== #
# DONE 1
class Practitioner(Info1):
...
class HumanName(Info1):
    # id_pembuat = models.ForeignKey('auth.User')
    description = ("Name of a human - parts and usage")
    # patient = models.ForeignKey('Patient', related_name='patientname', null=True, on_delete=models.CASCADE)
    patient = models.ForeignKey('Patient', null=True, on_delete=models.CASCADE)
    practitioner = models.ForeignKey('Practitioner', null=True, on_delete=models.CASCADE)
...
"""
NOTE:
no rekmed langsung jd satu dengan id pasien
